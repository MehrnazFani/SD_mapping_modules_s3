...
def performAngleMetrics(
    ...
    mean_accuracy = np.diag(hist) / (hist.sum(1)+eps)
    #mean_iou = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
    mean_iou = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist)+ eps)
    ...
    if write and is_train:
        ...
        #     )
        # )
        text = "[%d], Pixel Accuracy:%.3f, Mean Accuracy:%.3f, Mean IoU:%.3f, Freq.Weighted Accuray:%.3f  \n" \
            % (
                epoch,
                100 * pixel_accuracy,
                100 * np.nanmean(mean_accuracy),
        ...
        with open(train_loss_angle_file.name, "a") as f:
            f.write(text)
        ...
        with open(val_loss_angle_file.name, "a") as f:
            f.write(text)
...
def performMetrics(
    ...
    if write and is_train:
        ...
        #     )
        #                     )
        text = "[%d], Loss:%.5f, Loss(VecMap):%.5f, Pixel Accuracy:%.3f, Mean Accuracy:%.3f, Mean IoU:%.3f, Class IoU:[%.5f/%.5f], Freq.Weighted Accuray:%.3f  \n" \
            % (
                epoch,
                loss,
                loss_vec,
        ...
        with open(train_loss_file.name, "a") as f:
            f.write(text)
        ...
        with open(val_loss_file.name, "a") as f:
            f.write(text)
...
def save_checkpoint(epoch, loss, model, optimizer, best_accuracy, best_miou, config, experiment_dir):
    ...
    print(f"Saving current best model in : {experiment_dir}/model_best.pth.tar")

# Modified by Anmol
# Multiplied the input by 255
# 
def savePredictedProb(
    ...
    for idx in range(b):
        ...
        real_ = cv2.cvtColor(real_, cv2.COLOR_RGB2BGR) * 255
