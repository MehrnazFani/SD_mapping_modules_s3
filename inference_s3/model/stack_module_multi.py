...
class StackHourglassNetMTL_Multi(nn.Module):
    def __init__(
                self,
                task1_classes=2,
                task2_classes=37,
        ...
        super(StackHourglassNetMTL_Multi, self).__init__()

        self.inplanes = 64
        self.num_feats = 128
        self.num_stacks = num_stacks
        self.conv1 = nn.Conv2d(in_channels, self.inplanes, kernel_size=7, stride=2, padding=3, bias=True)
        ...
        self.angle_decoder1_score = nn.Conv2d(self.inplanes, task2_classes, kernel_size=1, bias=True)
    ...
    def function(self, x, rows, cols):
        out_1 = []
        out_2 = []

        for i in range(self.num_stacks): # Output: (1, 2, ?, 768) > This 2 comes from self.num_stacks=2
            y1, y2 = self.hg[i](x)
        ...
        d1 = self.decoder1(y1)[:, :, : int(math.ceil(rows / 2.0)), : int(math.ceil(cols / 2.0))]
        ...
        # Final Classification
        a_d1 = self.angle_decoder1(y2)[:, :, : int(math.ceil(rows / 2.0)), : int(math.ceil(cols / 2.0))]
    ...
    def forward(self, x):
        ...
        out_1_1, out_2_1 = self.function(x, rows, cols)
        out_1_2, out_2_2 = self.function(x, rows, cols)
        out_1_3, out_2_3 = self.function(x, rows, cols)

        return out_1_1, out_2_1, out_1_2, out_2_2, out_1_3, out_2_3
